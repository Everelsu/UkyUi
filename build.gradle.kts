...
// UTF-8 everywhere, rather than whatever the machine's default happens to be.
//
// Not cosmetic here: the sources carry Cyrillic in comments and string literals, and
// the language files are UTF-8 by Minecraft's own rules. javac and the resource copy
// both default to the platform encoding, which on a Russian Windows install is
// windows-1251 — so a build that works on one machine dies on "unmappable character"
// or, worse, silently ships mojibake in the shipped .lang.
